...
public class ARucoUnityHelper
{
    ...
    private static Color32Bytes colorData = new Color32Bytes();
    private static byte[] byteArray = new byte[0];
    private static Color32[] colorArray = new Color32[0];
    ...
    public static Texture2D MatToTexture(Mat mat, Texture2D outTexture = null)
    {
        ...
        using (Mat unityMat = new Mat(utils_mat_to_texture_2(mat.CvPtr)))
        {
            ...
            //only allocates a new array if the size changed so should be happening once.
            Array.Resize(ref byteArray, count);
            Array.Resize(ref colorArray, count);

            colorData.byteArray = byteArray;
            colorData.colors = colorArray;
            Marshal.Copy(unityMat.Data, colorData.byteArray, 0, colorData.byteArray.Length);
            outTexture.SetPixels32(colorData.colors);
            outTexture.Apply();
